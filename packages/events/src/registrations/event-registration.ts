...
export const registerEvent = (eventBus: EventBus) => {
    ...
    eventBus.subscribe(
        "submission.created",
        loggingHandler.handle.bind(loggingHandler),
    );

    // 📊 Registered exactly ONCE for accurate metrics
    eventBus.subscribe(
        "submission.created",
        metricsHandler.handle.bind(metricsHandler),
    );

    eventBus.subscribe(
        "submission.created",
        auditHandler.handle.bind(auditHandler),
    );
